...
from work_file import read_bytes, write_bytes, read_txt


class HybridCryptoSystem:
    """
    class implementing a hybrid crypto system using SEED (symmetric)
    and RSA (asymmetric) algorithms
    """
    ...
    def __init__(
        ...
        self.asymmetric = AsymmetricCryptography(private_key_path,
                                                 public_key_path)
    ...
    def generate_keys(self, size: int) -> None:
        ...
        try:
            ...
            Serialization.save_private_key(
                self.asymmetric.private_key_path, private_key
            )
            Serialization.save_public_key(
                self.asymmetric.public_key_path,
                public_key
            )
            write_bytes(
                public_key.encrypt(
    ...
    def encrypt(self, data_path: str, encrypted_data_path: str) -> None:
        ...
        try:
            ...
            key = Serialization.load_private_key(
